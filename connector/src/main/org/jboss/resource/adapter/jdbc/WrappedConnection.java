...
public class WrappedConnection implements Connection {
	...
	public void close() throws SQLException {
		...
		if (mc != null) {
			if (trackStatements != BaseWrapperManagedConnectionFactory.TRACK_STATEMENTS_FALSE_INT) {
				synchronized (this) {
					if (statements != null) {
						for (Iterator i = statements.entrySet().iterator(); i
								.hasNext();) {
							Map.Entry entry = (Map.Entry) i.next();
							WrappedStatement ws = (WrappedStatement) entry
									.getKey();
							if (trackStatements == BaseWrapperManagedConnectionFactory.TRACK_STATEMENTS_TRUE_INT) {
								Throwable stackTrace = (Throwable) entry
										.getValue();
								log.warn(
	...
	public void setClientInfo(Properties properties)
		...
		try {
			...
			throw new java.sql.SQLClientInfoException(sex.getMessage(),sex.getSQLState(),Collections.EMPTY_MAP);
		}
	}
	...
	public void setSchema(String schema) throws SQLException {
		mc.getConnection().setSchema(schema);
	}

	public String getSchema() throws SQLException {
		return mc.getConnection().getSchema();
	}
	...
	public void abort(java.util.concurrent.Executor executor)
			throws SQLException {
		mc.getConnection().abort(executor);
	}

	public Object  unwrap(Class iface) throws SQLException {
		return mc.getConnection().unwrap(iface);
	}

	public boolean isWrapperFor(Class iface) throws SQLException {

		return mc.getConnection().isWrapperFor(iface);
	}
	...
	public void setClientInfo(String name, String value)
		...
		try {
			...
			throw new SQLClientInfoException(sql.getMessage(),sql.getSQLState(),Collections.EMPTY_MAP);
		}
	...
	public void setNetworkTimeout(Executor executor, int milliseconds)
			throws SQLException {
		mc.getConnection().setNetworkTimeout(executor, milliseconds);
	}
